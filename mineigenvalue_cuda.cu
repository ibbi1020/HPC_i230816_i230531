 ...
 *********************************************************************/
     ...
     int window_hw)
 {
     ...
     // Robust discriminant: clamp to 0 to avoid sqrt of negative
     float disc2 = trace * trace - 4.0f * det;
     disc2 = fmaxf(0.0f, disc2);
     
     float min_eigenval = 0.5f * (trace - sqrtf(disc2));
     
     // Store only valid (finite) values
     if (!isfinite(min_eigenval)) {
         eigenvalues[y * ncols + x] = -1.0f;
     } else {
