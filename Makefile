...
######################################################################
# CUDA Configuration
# Set USE_CUDA=1 to enable GPU acceleration, USE_CUDA=0 for CPU only
# Example: make USE_CUDA=1 example3
USE_CUDA ?= 0
...
######################################################################
# Conditional CUDA setup based on USE_CUDA flag
ifeq ($(USE_CUDA), 1)
    # GPU mode: Add CUDA defines and include paths
    CUDA_DEF = -DUSE_CUDA
    CUDA_INCLUDE = -I/usr/local/cuda/include
    EXTRA_LIBS = $(CUDA_LIBDIR) $(CUDA_LIBS)
else
...
CFLAGS = $(FLAG1) $(FLAG2) $(CUDA_DEF) $(CUDA_INCLUDE)
######################################################################
# CUDA flags
# GTX 960 is Maxwell architecture with compute capability 5.2
NVCCFLAGS = $(FLAG1) $(FLAG2) $(CUDA_DEF) \
...
# CUDA source files (only compiled if USE_CUDA=1)
CUDA_SRCS = interpolate_cuda.cu mineigenvalue_cuda.cu convolve_gpu.cu subsample_cuda.cu

# Conditional object files based on USE_CUDA
ifeq ($(USE_CUDA), 1)
...
# CUDA compilation rules (only active if USE_CUDA=1)
ifeq ($(USE_CUDA), 1)
.cu.o:
	$(NVCC) -c $(CUDA_FLAGS) $<
...
subsample_cuda.o: subsample_cuda.cu subsample_cuda.h cuda_config.h
	$(NVCC) -c $(CUDA_FLAGS) subsample_cuda.cu
endif
...
example1: libklt.a
	$(CC) -O3 $(CFLAGS) -o $@ $@.c -L. -lklt $(LIB) $(EXTRA_LIBS) -lm
