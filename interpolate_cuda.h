...
/*********************************************************************
 * cudaNaiveInterpolateFullyPersistent
 * 
 * Fully persistent version - all buffers managed by caller.
 * Zero GPU allocations during runtime.
 * 
 * Parameters:
 *   d_img - device image (pre-allocated on GPU)
 *   width, height - image dimensions
 *   d_coords - device coordinates (pre-allocated on GPU)
 *   d_results - device results (pre-allocated on GPU)
 *   numPoints - number of points
 */
void cudaNaiveInterpolateFullyPersistent(
    const float* d_img,
    int width,
    int height,
    float* d_coords,
