...
typedef struct  {
  ...
  */
  ...
  /* GPU persistent buffers (only used when USE_CUDA is defined) */
  /* These buffers are allocated once and reused to avoid allocation overhead */
  
  /* Convolution buffers */
  void *gpu_temp_float_img1;    /* First temp buffer for convolution */
  void *gpu_temp_float_img2;    /* Second temp buffer for convolution */
  void *gpu_temp_float_img3;    /* Third temp buffer for convolution */
  size_t gpu_buffer_size;        /* Size in bytes of allocated convolution buffers */
  
  /* Interpolation buffers (for trackFeatures.c) */
  void *gpu_interp_img_data;     /* Device buffer for image data during interpolation */
  void *gpu_interp_coords;       /* Device buffer for feature coordinates (x,y pairs) */
  void *gpu_interp_results;      /* Device buffer for interpolation results */
  size_t gpu_interp_img_size;    /* Size of image buffer */
  size_t gpu_interp_max_points;  /* Maximum number of interpolation points */
  
  /* Feature selection buffers (for selectGoodFeatures.c) */
  void *gpu_gradx_data;          /* Device buffer for gradient X */
  void *gpu_grady_data;          /* Device buffer for gradient Y */
  void *gpu_eigenvalues;         /* Device buffer for computed eigenvalues */
  size_t gpu_eigen_buffer_size;  /* Size of gradient/eigenvalue buffers */
  
  /* GPU pyramid caching (for pyramid.c and trackFeatures.c) */
  /* These keep the entire image pyramid on GPU to avoid memory transfers */
  /* We need TWO pyramid sets since tracking compares pyramid1 vs pyramid2 */
  float **gpu_pyramid1_data;     /* Array of device pointers for pyramid1 intensity levels */
  float **gpu_gradx1_pyramid;    /* Array of device pointers for pyramid1 gradient X */
  float **gpu_grady1_pyramid;    /* Array of device pointers for pyramid1 gradient Y */
  float **gpu_pyramid2_data;     /* Array of device pointers for pyramid2 intensity levels */
  float **gpu_gradx2_pyramid;    /* Array of device pointers for pyramid2 gradient X */
  float **gpu_grady2_pyramid;    /* Array of device pointers for pyramid2 gradient Y */
  int gpu_pyramid_nlevels;       /* Number of levels in GPU pyramids */
  int *gpu_pyramid_ncols;        /* Host array: width of each pyramid level */
  int *gpu_pyramid_nrows;        /* Host array: height of each pyramid level */
  float *gpu_temp_kernel;        /* Device buffer for convolution kernels (max 71 floats) */
  int gpu_pyramid_initialized;   /* Flag: 1 if GPU pyramids are allocated, 0 otherwise */
  
  int gpu_buffers_initialized;   /* Flag: 1 if GPU buffers are allocated, 0 otherwise */
