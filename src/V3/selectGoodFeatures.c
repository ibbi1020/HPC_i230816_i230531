...
#ifdef USE_CUDA
    /* CUDA version with persistent GPU buffers */
    extern KLT_TrackingContext _KLTGlobalTrackingContext;
    KLT_TrackingContext tc_ref = _KLTGlobalTrackingContext;
    
    float* eigenvalues = (float*)malloc(ncols * nrows * sizeof(float));
    size_t img_size = ncols * nrows * sizeof(float);
    
    /* Allocate/reuse GPU buffers for gradients and eigenvalues */
    float *d_gradx, *d_grady, *d_eigenvalues;
    
    if (tc_ref && tc_ref->gpu_eigen_buffer_size >= img_size) {
        /* Fast path - reuse existing buffers */
        d_gradx = (float*)tc_ref->gpu_gradx_data;
        d_grady = (float*)tc_ref->gpu_grady_data;
        d_eigenvalues = (float*)tc_ref->gpu_eigenvalues;
    } else {
        /* Allocate new buffers with 20% padding */
        if (tc_ref && tc_ref->gpu_gradx_data) {
            cudaFree(tc_ref->gpu_gradx_data);
            cudaFree(tc_ref->gpu_grady_data);
            cudaFree(tc_ref->gpu_eigenvalues);
        }
        
        size_t alloc_size = img_size + img_size / 5;
        cudaMalloc((void**)&d_gradx, alloc_size);
        cudaMalloc((void**)&d_grady, alloc_size);
        cudaMalloc((void**)&d_eigenvalues, alloc_size);
        
        if (tc_ref) {
            tc_ref->gpu_gradx_data = d_gradx;
            tc_ref->gpu_grady_data = d_grady;
            tc_ref->gpu_eigenvalues = d_eigenvalues;
            tc_ref->gpu_eigen_buffer_size = alloc_size;
            tc_ref->gpu_buffers_initialized = 1;
        }
    }
    
    /* Upload gradients to GPU */
    cudaMemcpy(d_gradx, gradx->data, img_size, cudaMemcpyHostToDevice);
    cudaMemcpy(d_grady, grady->data, img_size, cudaMemcpyHostToDevice);
    
    /* Compute eigenvalues on GPU */
    cudaComputeMinEigenvaluesFullyPersistent(
        d_gradx, d_grady, d_eigenvalues,
        ncols, nrows, window_hw
    );
    
    /* Download results */
    cudaMemcpy(eigenvalues, d_eigenvalues, img_size, cudaMemcpyDeviceToHost);
