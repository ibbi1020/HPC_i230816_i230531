 ...
 * _uploadPyramidToGPU
 * 
 * Helper function to upload CPU-computed pyramids to GPU for caching.
 * This eliminates per-interpolation uploads by uploading entire pyramid once.
 * 
 * INPUT:
 *   tc: Tracking context (contains GPU pyramid storage)
 *   pyramid: CPU intensity pyramid to upload
 *   gradx_pyramid: CPU gradient X pyramid
 *   grady_pyramid: CPU gradient Y pyramid
 *   pyramid_id: 1 for pyramid1, 2 for pyramid2
 *   allocate: If 1, allocate GPU pyramids first (first-time setup)
 *********************************************************************/
static void _uploadPyramidToGPU(
    ...
    int allocate)
{
    // Allocate GPU pyramids if needed (first time only)
    if (allocate && !tc->gpu_pyramid_initialized) {
        int base_ncols = pyramid->ncols[0];
        int base_nrows = pyramid->nrows[0];
        int nlevels = pyramid->nLevels;
        
        // Allocate host arrays for dimensions
        tc->gpu_pyramid_ncols = (int*)malloc(nlevels * sizeof(int));
        tc->gpu_pyramid_nrows = (int*)malloc(nlevels * sizeof(int));
        
        // Allocate host arrays of device pointers for BOTH pyramids
        tc->gpu_pyramid1_data = (float**)malloc(nlevels * sizeof(float*));
        tc->gpu_gradx1_pyramid = (float**)malloc(nlevels * sizeof(float*));
        tc->gpu_grady1_pyramid = (float**)malloc(nlevels * sizeof(float*));
        tc->gpu_pyramid2_data = (float**)malloc(nlevels * sizeof(float*));
        tc->gpu_gradx2_pyramid = (float**)malloc(nlevels * sizeof(float*));
        tc->gpu_grady2_pyramid = (float**)malloc(nlevels * sizeof(float*));
        
        if (!tc->gpu_pyramid_ncols || !tc->gpu_pyramid_nrows ||
            !tc->gpu_pyramid1_data || !tc->gpu_gradx1_pyramid || !tc->gpu_grady1_pyramid ||
            !tc->gpu_pyramid2_data || !tc->gpu_gradx2_pyramid || !tc->gpu_grady2_pyramid) {
            fprintf(stderr, "ERROR: Failed to allocate host arrays for GPU pyramids\n");
            return;
        }
        
        // Allocate device memory for each level of BOTH pyramids
        int ncols = base_ncols;
        int nrows = base_nrows;
        
        for (int i = 0; i < nlevels; i++) {
            tc->gpu_pyramid_ncols[i] = ncols;
            tc->gpu_pyramid_nrows[i] = nrows;
            
            size_t level_size = ncols * nrows * sizeof(float);
            cudaError_t err;
            
            // Allocate pyramid1 level (intensity + gradients)
            err = cudaMalloc((void**)&tc->gpu_pyramid1_data[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for pyramid1 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
            err = cudaMalloc((void**)&tc->gpu_gradx1_pyramid[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for gradx1 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
            err = cudaMalloc((void**)&tc->gpu_grady1_pyramid[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for grady1 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
            
            // Allocate pyramid2 level (intensity + gradients)
            err = cudaMalloc((void**)&tc->gpu_pyramid2_data[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for pyramid2 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
            err = cudaMalloc((void**)&tc->gpu_gradx2_pyramid[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for gradx2 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
            err = cudaMalloc((void**)&tc->gpu_grady2_pyramid[i], level_size);
            if (err != cudaSuccess) {
                fprintf(stderr, "ERROR: cudaMalloc failed for grady2 level %d: %s\n", 
                        i, cudaGetErrorString(err));
                return;
            }
        ...
        tc->gpu_pyramid_nlevels = nlevels;
        tc->gpu_pyramid_initialized = 1;
        
        // Diagnostic message about GPU pyramid allocation
        printf("  [GPU Pyramid Cache] Allocated: %d levels, base %dx%d\n", 
               nlevels, base_ncols, base_nrows);
    }
    
    // Upload pyramid data to GPU
    if (tc->gpu_pyramid_initialized) {
        float **target_pyramid_data;
        float **target_gradx_pyramid;
        float **target_grady_pyramid;
        
        // Select target pyramid based on pyramid_id
        if (pyramid_id == 1) {
            target_pyramid_data = tc->gpu_pyramid1_data;
            target_gradx_pyramid = tc->gpu_gradx1_pyramid;
            target_grady_pyramid = tc->gpu_grady1_pyramid;
        } else {
            target_pyramid_data = tc->gpu_pyramid2_data;
            target_gradx_pyramid = tc->gpu_gradx2_pyramid;
            target_grady_pyramid = tc->gpu_grady2_pyramid;
        }
        
        // Upload all levels
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
            size_t level_size = tc->gpu_pyramid_ncols[i] * tc->gpu_pyramid_nrows[i] * sizeof(float);
            
            cudaMemcpy(target_pyramid_data[i], pyramid->img[i]->data, 
                       level_size, cudaMemcpyHostToDevice);
            cudaMemcpy(target_gradx_pyramid[i], gradx_pyramid->img[i]->data,
                       level_size, cudaMemcpyHostToDevice);
            cudaMemcpy(target_grady_pyramid[i], grady_pyramid->img[i]->data,
                       level_size, cudaMemcpyHostToDevice);
        }
...
#ifdef USE_CUDA
/*********************************************************************
 * Global pyramid tracking for GPU cache detection
 * 
 * These globals allow interpolation functions to detect if images are
 * from cached GPU pyramids and use GPU pointers directly instead of
 * uploading data. Set before tracking loop, updated per pyramid level.
 *********************************************************************/
static _KLT_Pyramid _active_pyramid1 = NULL;
static _KLT_Pyramid _active_pyramid1_gradx = NULL;
static _KLT_Pyramid _active_pyramid1_grady = NULL;
static _KLT_Pyramid _active_pyramid2 = NULL;
static _KLT_Pyramid _active_pyramid2_gradx = NULL;
static _KLT_Pyramid _active_pyramid2_grady = NULL;
static int _active_pyramid_level = -1;
#endif
 ...
 *
 * CUDA-accelerated version using fully persistent GPU buffers.
 * Minimizes allocations and data transfers.
 */
  ...
  _FloatWindow imgdiff)
{
  ...
  int numPoints = windowSize * 2;
  size_t img_size = img1->ncols * img1->nrows * sizeof(float);
  size_t coords_size = numPoints * 2 * sizeof(float);
  size_t results_size = numPoints * sizeof(float);
  
  /* Host buffers for coordinate preparation and result collection */
  float *coords = (float*)malloc(coords_size);
  ...
  /* Allocate/reuse GPU buffers */
  float *d_img, *d_coords, *d_results;
  int needs_alloc = 0;
  
  if (tc && tc->gpu_interp_img_size >= img_size && 
      tc->gpu_interp_max_points >= numPoints) {
    /* Fast path - reuse existing */
    d_img = (float*)tc->gpu_interp_img_data;
    d_coords = (float*)tc->gpu_interp_coords;
    d_results = (float*)tc->gpu_interp_results;
  } else {
    needs_alloc = 1;
    if (tc && tc->gpu_interp_img_data) {
      cudaFree(tc->gpu_interp_img_data);
      cudaFree(tc->gpu_interp_coords);
      cudaFree(tc->gpu_interp_results);
    }
    
    size_t alloc_img_size = img_size + img_size / 5;
    size_t alloc_points = numPoints + numPoints / 5;
    
    cudaMalloc((void**)&d_img, alloc_img_size);
    cudaMalloc((void**)&d_coords, alloc_points * 2 * sizeof(float));
    cudaMalloc((void**)&d_results, alloc_points * sizeof(float));
    
    if (tc) {
      tc->gpu_interp_img_data = d_img;
      tc->gpu_interp_coords = d_coords;
      tc->gpu_interp_results = d_results;
      tc->gpu_interp_img_size = alloc_img_size;
      tc->gpu_interp_max_points = alloc_points;
      tc->gpu_buffers_initialized = 1;
    }
  }
  
  /* Process img1: upload image, upload coords, compute, download results */
  float *d_img1_to_use, *d_img2_to_use;
  int img1_is_cached = 0, img2_is_cached = 0;
  
  /* PYRAMID CACHE DETECTION: Check if img1 is from the active pyramid1 */
  if (_active_pyramid1 != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      img1->data == _active_pyramid1->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU pyramid data directly, no upload needed */
    d_img1_to_use = tc->gpu_pyramid1_data[_active_pyramid_level];
    img1_is_cached = 1;
  } else {
    /* Cache miss: upload image to GPU buffer */
    cudaMemcpy(d_img, img1->data, img_size, cudaMemcpyHostToDevice);
    d_img1_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_img1_to_use, img1->ncols, img1->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
  
  /* Process img2: upload image, upload coords, compute, download results */
  /* PYRAMID CACHE DETECTION: Check if img2 is from the active pyramid2 */
  if (_active_pyramid2 != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      img2->data == _active_pyramid2->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU pyramid data directly, no upload needed */
    d_img2_to_use = tc->gpu_pyramid2_data[_active_pyramid_level];
    img2_is_cached = 1;
  } else {
    /* Cache miss: upload image to GPU buffer */
    cudaMemcpy(d_img, img2->data, img_size, cudaMemcpyHostToDevice);
    d_img2_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords + windowSize * 2, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_img2_to_use, img2->ncols, img2->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results + windowSize, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
 ...
 *
 * CUDA-accelerated version using fully persistent GPU buffers.
 * Processes 4 gradient images with minimal allocations/transfers.
 */
  ...
  _FloatWindow grady)
{
  ...
  int hw = width/2, hh = height/2;
  int windowSize = width * height;
  int numPoints = windowSize * 4;
  size_t img_size = gradx1->ncols * gradx1->nrows * sizeof(float);
  size_t coords_size = numPoints * 2 * sizeof(float);
  size_t results_size = numPoints * sizeof(float);
  ...
  /* Prepare coordinates for all 4 gradients */
  int idx = 0;
  for (int j = -hh; j <= hh; j++) {
    for (int i = -hw; i <= hw; i++) {
      coords[idx * 2] = x1 + i;
  ...
  /* Allocate/reuse GPU buffers (same as intensity difference) */
  float *d_img, *d_coords, *d_results;
  ...
  /* Process gradx1 */
  float *d_gradx1_to_use, *d_gradx2_to_use, *d_grady1_to_use, *d_grady2_to_use;
  int gradx1_is_cached = 0, gradx2_is_cached = 0, grady1_is_cached = 0, grady2_is_cached = 0;
  
  /* GRADIENT PYRAMID CACHE DETECTION: Check if gradx1 is from the active gradient pyramid */
  if (_active_pyramid1_gradx != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      gradx1->data == _active_pyramid1_gradx->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU gradient pyramid data directly */
    d_gradx1_to_use = tc->gpu_gradx1_pyramid[_active_pyramid_level];
    gradx1_is_cached = 1;
  } else {
    /* Cache miss: upload gradient to GPU buffer */
    cudaMemcpy(d_img, gradx1->data, img_size, cudaMemcpyHostToDevice);
    d_gradx1_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_gradx1_to_use, gradx1->ncols, gradx1->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
  
  /* Process gradx2 */
  /* GRADIENT PYRAMID CACHE DETECTION: Check if gradx2 is from the active gradient pyramid */
  if (_active_pyramid2_gradx != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      gradx2->data == _active_pyramid2_gradx->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU gradient pyramid data directly */
    d_gradx2_to_use = tc->gpu_gradx2_pyramid[_active_pyramid_level];
    gradx2_is_cached = 1;
  } else {
    /* Cache miss: upload gradient to GPU buffer */
    cudaMemcpy(d_img, gradx2->data, img_size, cudaMemcpyHostToDevice);
    d_gradx2_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords + windowSize * 2, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_gradx2_to_use, gradx2->ncols, gradx2->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results + windowSize, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
  
  /* Process grady1 */
  /* GRADIENT PYRAMID CACHE DETECTION: Check if grady1 is from the active gradient pyramid */
  if (_active_pyramid1_grady != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      grady1->data == _active_pyramid1_grady->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU gradient pyramid data directly */
    d_grady1_to_use = tc->gpu_grady1_pyramid[_active_pyramid_level];
    grady1_is_cached = 1;
  } else {
    /* Cache miss: upload gradient to GPU buffer */
    cudaMemcpy(d_img, grady1->data, img_size, cudaMemcpyHostToDevice);
    d_grady1_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords + windowSize * 4, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_grady1_to_use, grady1->ncols, grady1->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results + windowSize * 2, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
  
  /* Process grady2 */
  /* GRADIENT PYRAMID CACHE DETECTION: Check if grady2 is from the active gradient pyramid */
  if (_active_pyramid2_grady != NULL && _active_pyramid_level >= 0 && 
      tc->gpu_pyramid_initialized && 
      grady2->data == _active_pyramid2_grady->img[_active_pyramid_level]->data) {
    /* Cache hit! Use GPU gradient pyramid data directly */
    d_grady2_to_use = tc->gpu_grady2_pyramid[_active_pyramid_level];
    grady2_is_cached = 1;
  } else {
    /* Cache miss: upload gradient to GPU buffer */
    cudaMemcpy(d_img, grady2->data, img_size, cudaMemcpyHostToDevice);
    d_grady2_to_use = d_img;
  }
  
  cudaMemcpy(d_coords, coords + windowSize * 6, windowSize * 2 * sizeof(float), cudaMemcpyHostToDevice);
  cudaNaiveInterpolateFullyPersistent(d_grady2_to_use, grady2->ncols, grady2->nrows, d_coords, d_results, windowSize);
  cudaMemcpy(results + windowSize * 3, d_results, windowSize * sizeof(float), cudaMemcpyDeviceToHost);
					  ...
					  KLT_FeatureList featurelist)
{
	...
	/* Set global context for persistent GPU buffers */
	_KLTSetGlobalTrackingContext(tc);
...
#ifdef USE_CUDA
		/* CRITICAL: In sequential mode, pyramid1 is now the OLD pyramid2 from the previous frame.
		 * On the CPU side, the pointer assignment above (pyramid1 = tc->pyramid_last) makes this work.
		 * On the GPU side, we need to SWAP the pyramid slot pointers so that:
		 *   - gpu_pyramid1_data now points to what WAS gpu_pyramid2_data (the old frame, now current)
		 *   - gpu_pyramid2_data becomes available for the NEW incoming frame
		 * Without this swap, pyramid1 would point to the wrong GPU data, causing incorrect tracking. */
		if (tc->gpu_pyramid_initialized) {
			/* Swap pyramid data pointers */
			float **temp_data = tc->gpu_pyramid1_data;
			tc->gpu_pyramid1_data = tc->gpu_pyramid2_data;
			tc->gpu_pyramid2_data = temp_data;
			
			/* Swap gradient X pointers */
			float **temp_gradx = tc->gpu_gradx1_pyramid;
			tc->gpu_gradx1_pyramid = tc->gpu_gradx2_pyramid;
			tc->gpu_gradx2_pyramid = temp_gradx;
			
			/* Swap gradient Y pointers */
			float **temp_grady = tc->gpu_grady1_pyramid;
			tc->gpu_grady1_pyramid = tc->gpu_grady2_pyramid;
			tc->gpu_grady2_pyramid = temp_grady;
		}
#endif
	} else  {
...
#ifdef USE_CUDA
		/* Upload pyramid1 to GPU (first time allocation + upload) */
		_uploadPyramidToGPU(tc, pyramid1, pyramid1_gradx, pyramid1_grady, 1, 1);
#endif
	}
...
#ifdef USE_CUDA
	/* Upload pyramid2 to GPU (allocate if needed, always upload as pyramid2) */
	_uploadPyramidToGPU(tc, pyramid2, pyramid2_gradx, pyramid2_grady, 2,
	                    tc->gpu_pyramid_initialized ? 0 : 1);
	
	/* Set global pyramid pointers for interpolation cache detection.
	 * These globals allow _computeIntensityDifferenceCUDA and _computeGradientSumCUDA
	 * to detect pyramid images via pointer comparison and use cached GPU data directly,
	 * eliminating redundant cudaMemcpy calls (133,221 transfers → ~100 per frame). */
	if (tc->gpu_pyramid_initialized) {
		_active_pyramid1 = pyramid1;
		_active_pyramid1_gradx = pyramid1_gradx;
		_active_pyramid1_grady = pyramid1_grady;
		_active_pyramid2 = pyramid2;
		_active_pyramid2_gradx = pyramid2_gradx;
		_active_pyramid2_grady = pyramid2_grady;
		/* _active_pyramid_level will be updated in the tracking loop */
	}
#endif
...
#ifdef USE_CUDA
				/* Update active pyramid level for GPU cache detection.
				 * This allows interpolation functions to know which GPU pyramid level to use. */
				_active_pyramid_level = r;
#endif
