 ...
 * 
 * Fully persistent version - all buffers on GPU, managed by caller.
 * Zero GPU allocations during runtime.
 * 
 * Parameters:
 *   d_gradx - device gradient x (pre-allocated on GPU)
 *   d_grady - device gradient y (pre-allocated on GPU)
 *   d_eigenvalues - device eigenvalues (pre-allocated on GPU)
 *   ncols, nrows - image dimensions
 *   window_hw - half-width of window
 */
void cudaComputeMinEigenvaluesFullyPersistent(
    const float* d_gradx,
    const float* d_grady,
    float* d_eigenvalues,
    int ncols,
