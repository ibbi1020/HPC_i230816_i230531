...
/* CUDA runtime for persistent buffers */
#ifdef USE_CUDA
...
KLT_TrackingContext KLTCreateTrackingContext()
{
  ...
  /* Initialize ALL GPU buffer fields to NULL */
  /* Convolution buffers */
  tc->gpu_temp_float_img1 = NULL;
  tc->gpu_temp_float_img2 = NULL;
  tc->gpu_temp_float_img3 = NULL;
  tc->gpu_buffer_size = 0;
  
  /* Interpolation buffers */
  tc->gpu_interp_img_data = NULL;
  tc->gpu_interp_coords = NULL;
  tc->gpu_interp_results = NULL;
  tc->gpu_interp_img_size = 0;
  tc->gpu_interp_max_points = 0;
  
  /* Feature selection buffers */
  tc->gpu_gradx_data = NULL;
  tc->gpu_grady_data = NULL;
  tc->gpu_eigenvalues = NULL;
  tc->gpu_eigen_buffer_size = 0;
  
  /* GPU pyramid caching */
  tc->gpu_pyramid1_data = NULL;
  tc->gpu_gradx1_pyramid = NULL;
  tc->gpu_grady1_pyramid = NULL;
  tc->gpu_pyramid2_data = NULL;
  tc->gpu_gradx2_pyramid = NULL;
  tc->gpu_grady2_pyramid = NULL;
  tc->gpu_pyramid_nlevels = 0;
  tc->gpu_pyramid_ncols = NULL;
  tc->gpu_pyramid_nrows = NULL;
  tc->gpu_temp_kernel = NULL;
  tc->gpu_pyramid_initialized = 0;
  
  tc->gpu_buffers_initialized = 0;
  ...
  KLT_TrackingContext tc)
{
  ...
  /* Free ALL GPU buffers if allocated */
#ifdef USE_CUDA
  if (tc->gpu_buffers_initialized) {
    /* Convolution buffers */
    if (tc->gpu_temp_float_img1) cudaFree(tc->gpu_temp_float_img1);
    if (tc->gpu_temp_float_img2) cudaFree(tc->gpu_temp_float_img2);
    if (tc->gpu_temp_float_img3) cudaFree(tc->gpu_temp_float_img3);
    
    /* Interpolation buffers */
    if (tc->gpu_interp_img_data) cudaFree(tc->gpu_interp_img_data);
    if (tc->gpu_interp_coords) cudaFree(tc->gpu_interp_coords);
    if (tc->gpu_interp_results) cudaFree(tc->gpu_interp_results);
    
    /* Feature selection buffers */
    if (tc->gpu_gradx_data) cudaFree(tc->gpu_gradx_data);
    if (tc->gpu_grady_data) cudaFree(tc->gpu_grady_data);
    if (tc->gpu_eigenvalues) cudaFree(tc->gpu_eigenvalues);
    
    /* GPU pyramid caching - free all pyramid levels */
    if (tc->gpu_pyramid_initialized) {
      // Free pyramid1 levels
      if (tc->gpu_pyramid1_data) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_pyramid1_data[i]) cudaFree(tc->gpu_pyramid1_data[i]);
        }
        free(tc->gpu_pyramid1_data);
      }
      if (tc->gpu_gradx1_pyramid) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_gradx1_pyramid[i]) cudaFree(tc->gpu_gradx1_pyramid[i]);
        }
        free(tc->gpu_gradx1_pyramid);
      }
      if (tc->gpu_grady1_pyramid) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_grady1_pyramid[i]) cudaFree(tc->gpu_grady1_pyramid[i]);
        }
        free(tc->gpu_grady1_pyramid);
      }
      
      // Free pyramid2 levels
      if (tc->gpu_pyramid2_data) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_pyramid2_data[i]) cudaFree(tc->gpu_pyramid2_data[i]);
        }
        free(tc->gpu_pyramid2_data);
      }
      if (tc->gpu_gradx2_pyramid) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_gradx2_pyramid[i]) cudaFree(tc->gpu_gradx2_pyramid[i]);
        }
        free(tc->gpu_gradx2_pyramid);
      }
      if (tc->gpu_grady2_pyramid) {
        for (int i = 0; i < tc->gpu_pyramid_nlevels; i++) {
          if (tc->gpu_grady2_pyramid[i]) cudaFree(tc->gpu_grady2_pyramid[i]);
        }
        free(tc->gpu_grady2_pyramid);
      }
      
      // Free dimension arrays
      if (tc->gpu_pyramid_ncols) free(tc->gpu_pyramid_ncols);
      if (tc->gpu_pyramid_nrows) free(tc->gpu_pyramid_nrows);
      
      // Free kernel buffer
      if (tc->gpu_temp_kernel) cudaFree(tc->gpu_temp_kernel);
